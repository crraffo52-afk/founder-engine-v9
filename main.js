...
window.switchTab = function(tab) {
  ...
  const global = byId('tabGlobalContent');
  const pre  = byId('tabPreContent');
  const tracker = byId('tabTrackerContent');
  
  const btnLive = byId('tabLive');
  const btnGlobal = byId('tabGlobal');
  const btnPre  = byId('tabPre');
  const btnTracker = byId('tabTracker');

  [live, global, pre, tracker].forEach(e => e && (e.style.display = 'none'));
  [btnLive, btnGlobal, btnPre, btnTracker].forEach(e => e && e.classList.remove('active'));
  ...
  if (tab === 'live') {
    ...
    if (typeof window.fetchGlobalScanner === 'function') window.fetchGlobalScanner();
  } else if (tab === 'pre') {
    pre.style.display = '';
